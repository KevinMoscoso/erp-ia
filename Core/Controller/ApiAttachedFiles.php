...
class ApiAttachedFiles extends ApiController
{
    ...
    public function realizarDELETE(): void
    {
        if (empty($this->obtenerParametroUri(3)) || $this->modelo->cargar($this->obtenerParametroUri(3)) === false) {
            $this->establecerError(Tools::trans('registro-no-encontrado'), null, Response::HTTP_NO_ENCONTRADO);
            return;
        }

        if ($this->modelo->eliminar() === false) {
            $this->establecerError(Tools::trans('error-eliminar-registro'), null, Response::HTTP_CONFLICTO);
            return;
        }

        $this->establecerExito(Tools::trans('registro-eliminado-correctamente'), $this->modelo->aArray());
    }
    ...
    public function realizarPOST(): void
    {
        ...
        if ($this->modelo->cargar($codigo)) {
            $this->establecerError(Tools::trans('registro-duplicado'), $this->modelo->aArray(), Response::HTTP_CONFLICTO);
            return;
        } elseif (empty($valores) && empty($archivos)) {
            $this->establecerError(Tools::trans('sin-datos-recibidos'));
            return;
        }
    ...
    protected function listarTodos(): void
    {
        ...
        $orden = $this->solicitud->consulta->obtenerArray('orden');
    ...
    private function guardarRecurso(): void
    {
        if ($this->modelo->guardar()) {
            $this->establecerExito(Tools::trans('registro-actualizado-correctamente'), $this->modelo->aArray());
            return;
        }
        ...
        $this->establecerError($mensaje, $this->modelo->aArray(), Response::HTTP_ENTIDAD_NO_PROCESABLE);
    }
