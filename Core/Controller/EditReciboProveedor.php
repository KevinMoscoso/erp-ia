 ...
 * Controlador para la edición de recibos de proveedores
 * 
...
use ERPIA\Dinamic\Lib\Accounting\SupplierPaymentAccounting;
use ERPIA\Dinamic\Model\SupplierPayment;
...
class EditReciboProveedor extends EditController
{
     ...
     */
    protected function createPaymentsListView(string $viewName = 'ListPagoProveedor'): void
    {
        $this->addListView($viewName, 'PagoProveedor', 'pagos');
    ...
    protected function generateAccountingAction(): void
    {
        ...
        foreach ($selectedCodes as $paymentCode) {
            $paymentRecord = new SupplierPayment();
            ...
            $accountingTool = new SupplierPaymentAccounting();
    ...
    protected function loadData(string $viewName, BaseView $view): void
    {
        switch ($viewName) {
            case 'ListPagoProveedor':
                $receiptId = $this->getViewModelValue('EditReciboProveedor', 'idrecibo');
