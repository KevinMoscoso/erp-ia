...
final class IpBannedOnApi extends ErrorController
{
    /**
     * Ejecuta el controlador de error para IP bloqueada en la API.
     * Devuelve una respuesta JSON con código HTTP 429 (Too Many Requests).
     */
    public function run(): void
    {
        $this->response()
            ->setHttpCode(Response::HTTP_TOO_MANY_REQUESTS)
            ->json([
                'estado' => 'error',
                'mensaje' => $this->excepcion ? $this->excepcion->getMessage() : 'IP bloqueada por exceso de solicitudes',
            ]);
    }
}