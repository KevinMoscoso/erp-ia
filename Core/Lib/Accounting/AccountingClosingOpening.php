...
use ERPIA\Dinamic\Model\Ejercicio as DinEjercicio;
...
class AccountingClosingOpening extends AccountingClosingBase
{
    ...
    {
        return Tools::trans('opening-concept', [
    ...
    protected function getSQL(): string
    {
        ...
        $closingOperation = Asiento::OPERATION_CLOSING;
        ...
        if ($dbType == 'postgresql') {
            return "SELECT COALESCE(t1.canal, 0) AS channel,"
                ...
                . " AND (t1.operacion IS NULL OR t1.operacion <> '" . $closingOperation . "')"
                . " GROUP BY 1, 2, 3, 4"
    ...
    private function copySubAccount($idSubAccount): ?int
    {
        ...
        $subAccount->loadById($idSubAccount);
    ...
    private function copyAccounts(): bool
    {
        ...
        foreach (Cuenta::getAll($where, ['codcuenta' => 'ASC']) as $account) {
        ...
        // copy subaccounts
        foreach (Subcuenta::getAll($where, ['codsubcuenta' => 'ASC']) as $subaccount) {
            $newSubaccount = $accounting->copySubAccountToExercise($subaccount, $this->newExercise->codejercicio);
    ...
    private function setResultAccountData(&$data): void
    {
        ...
        $accounting->currentExercise = $this->newExercise;
