...
class AccountingCreation
{
    ...
    public function copyAccountToExercise($account, $codejercicio): Cuenta
    {
        ...
        if ($newAccount->loadFromConditions($conditions)) {
            return $newAccount;
        }
    ...
    public function copySubAccountToExercise($subAccount, $codejercicio): Subcuenta
    {
        ...
        if ($newSubaccount->loadFromConditions($conditions)) {
    ...
    public function createSubjectAccount(&$subject, $account): Subcuenta
    {
        ...
        if (false === $subaccount->save()) {
            Tools::log()->error('cannot-create-subaccount', [
    ...
    public function getFreeSubjectSubaccount($subject, $account): string
    {
        if (false === $this->checkExercise($account->codejercicio)) {
            Tools::log()->error('exercise-closed', ['%code%' => $account->codejercicio]);
            return '';
        }

        // keep only numbers from the code
        $code = preg_replace('/[^0-9]/', '', $subject->id());
        if (strlen($code) === $this->exercise->longsubcuenta) {
            // if the code already has the length of a subaccount, use it as subaccount
            return $code;
        }

        // build an array with the customer number, the first 49 numbers and several random numbers
        $numbers = array_merge(
            [$code],
            range(1, 49),
        ...
        // also add the next 100 numbers to the total subaccounts
        ...
        // try numbers to choose the first subaccount code that doesn't exist
        foreach ($numbers as $num) {
            ...
            // check that this subaccount is not in use by another customer or supplier
            $conditions = [new DataBaseWhere('codsubcuenta', $newCode)];
            $count = $subject->count($conditions);
            if ($count > 0) {
                continue;
            }

            // if the subaccount doesn't exist, choose it
            $conditions = [
                new DataBaseWhere('codejercicio', $account->codejercicio),
                new DataBaseWhere('codsubcuenta', $newCode)
            ];
            if (false === $subcuenta->loadFromConditions($conditions)) {
                return $newCode;
            }
        }

        // no free subaccount found
