...
abstract class ListController extends BaseController
{
    ...
    protected function exportAction()
    {
        if (false === $this->views[$this->active]->settings['btnPrint'] ||
            ...
            Logger::warning('no-print-permission');
            return;
        }
        $this->setTemplate(false);
    ...
    protected function getOwnerFilter($model): array
    {
        ...
        if (property_exists($model, 'username')) {
            ...
            // result: (`username` = 'username' OR `username` IS NULL OR `agent_code` = 'agent') AND [... user filters]
            $where[] = new DataBaseWhere('username', $this->user->username);
            $where[] = new DataBaseWhere('username', null, 'IS', 'OR');
            if (property_exists($model, 'agent_code') && $this->user->agent_code) {
                $where[] = new DataBaseWhere('agent_code', $this->user->agent_code, '=', 'OR');
            }
            return $where;
        }
        if (property_exists($model, 'agent_code')) {
            $where[] = new DataBaseWhere('agent_code', $this->user->agent_code);
        }
        return $where;
    }
    ...
    private function megaSearchColumns($view): array
    {
        ...
        foreach ($view->getColumns() as $col) {
            if (false === $col->hidden()) {
                $result[] = Translation::translate($col->title);
            }
        }
        return $result;
    }
